...
public class PuestoController {
    ...
    public ResponseEntity<Puesto> createPuesto(@RequestBody Puesto puesto) {
        if (puesto.getTipo() == null || tipoPuestoRepository.findById(puesto.getTipo().getId()).isEmpty()) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Puesto> updatePuesto(@PathVariable Long id, @RequestBody Puesto puestoDetails) {
        return puestoRepository.findById(id)
                .map(puesto -> {
